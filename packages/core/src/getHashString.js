...
export default (
	...
	const { label = '', ...hashableObject } = object
	...
	for (var value = JSON.stringify(hashableObject), index = value.length, hash = 9; index; ) {
		hash = Math.imul(hash ^ value.charCodeAt(--index), 9 ** 9)
	}

	return prefix + (hash ^ (hash >>> 9)).toString(36).slice(-5) + (label ? `-${label}` : '')
}
